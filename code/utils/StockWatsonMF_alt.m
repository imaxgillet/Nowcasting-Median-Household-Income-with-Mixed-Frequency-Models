function [factor, Gamma, Sigma, YHat, XHat, A] = StockWatsonMF_alt(X, varargin)
...
nSeries = size(X, 2);
nFactors = opts.factors;

% Initialize balanced panel as zeros, starting point shouldn't matter
YHat = X(balanceInx, :);
YHat(isnan(YHat)) = 0;

Gamma = zeros(nSeries,nFactors);
gap = 1e10; SSR0 = 1e10; iter = -1;
while abs(gap) > opts.tol
  iter = iter+1;
  
  % Take factors from PCA using fitted high-frequency data
  [Gamma, factor] = pca(YHat, 'NumComponents', nFactors, ...
    'Centered', false);
  
  % New projected high frequency data
  XBal = factor*Gamma';
  
  % Get corrected projected high frequency data 
  for iS = 1:nSeries
    cleanData(isnan(X(balanceInx, iS)),iS) = XBal(isnan(X(balanceInx, iS)),iS);
    cleanData(:,iS) = stddm(cleanData(:,iS));
    aggSer{iS} = Aselect{aggType(iS)} * cleanData(:,iS);
    YHat(:,iS) = enforceAgg(aggSer{iS}, factor *  Gamma(iS,:)', ...
      A{aggType(iS)}, errAdj{aggType(iS)}, hasHiFreqInfo(:,iS)); % Imposes temporal aggregation constraints
  end
...
if nargout >= 5
  ...
  for iS = 1:nSeries
    if aggType(iS)<2  
        ...
        XHat(:, iS) = Aselect{aggType(iS)}' * A{aggType(iS)} * YHat(:, iS);
        XHat(~sum(Aselect{aggType(iS)}), iS) = nan;
        XHat(:,iS) = XHat(:,iS)./sqrt(nanvar(XHat(:,iS))); % Rescales to natural frequency
    end
  end
...
% Construct aggregation and selection matrices from the Harvey-style
...
defAccum = struct('index', [], 'calendar', [], 'horizon', []);
inP.addParameter('accum', defAccum, @isstruct);
inP.addParameter('tol', 1e-6, @isnumeric);
inP.addParameter('verbose', true, @islogical);
inP.addParameter('rotate', false, @islogical);
inP.parse(opts, vararginCell{:});
opts = inP.Results;

% Validate accum struct
if isfield(opts.accum, 'xi')
  assert(isfield(opts.accum, 'psi'));
  assert(isfield(opts.accum, 'Horizon'));
  newAccum.index = opts.accum.xi;
  newAccum.calendar = opts.accum.psi;
  newAccum.horizon = opts.accum.Horizon;
  opts.accum = newAccum;
end
