function [dataDates, incomeEstimates, bic, hqic] = estimate_targeted_ar1_model(inputStructure, varargin)
...
%% Set up Model
Z = [nan(nTargets, nFactors) zeros(nTargets, nFactors*(nLags-1)) 1; ...
    eye(nFactors) zeros(nFactors, nFactors*(nLags-1))  zeros(nFactors,1)] ;

H = [zeros(nTargets) zeros(nTargets, nFactors); zeros(nFactors, nTargets) nan(nFactors)];

T = blkdiag([nan(nFactors*nLags, nFactors) ...
    [eye(nFactors*(nLags-1)); ...
    zeros(nFactors, nFactors*(nLags-1))]], nan);
T = T';
R = blkdiag([eye(nFactors); zeros((nLags-1)*nFactors, nFactors)],1); 

Q = blkdiag(eye(nFactors), nan); % diag(nan(1,nFactors));
